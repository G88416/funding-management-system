...
- Deploy the Firestore and Storage security rules from this repo using the Firebase CLI (`firebase deploy --only firestore:rules,storage`), which reads `firebase.json`.
- Firestore rules allow public reads for funders and contract leads but require authenticated users for writes and analytics collections. Storage rules require authenticated access for all paths.
